...
All non-B0 [INT](https://conwaylife.com/wiki/Isotropic_non-totalistic_rule) rules are supported, and are compiled to boolean arrays at runtime by the code in [hensel.hpp](/includes/hensel.hpp).
## Usage
The library contains a C++ class, ```Pattern```, in [pattern.hpp](/pattern.hpp), which can be manipulated using a range of methods. The class supports two string formats for initialisation - [RLE](https://conwaylife.com/wiki/Run_Length_Encoded) (recommended for most cases) and [apgcodes](https://conwaylife.com/wiki/Apgcode) (useful in occasional cases), with the rule being specified as a secondary argument.

Example code:
```// Create a pattern named pt using the RLE of the R-pentomino and rule B3/S23:
Pattern pt("o$3o$bo!", "b3s23");
// Advance it by four generations:
pt = pt[4];
```
The library can be used on both POSIX and Windows systems, but is much faster and easier to compile on the former.
## Data structure
...
## Python bindings
Python bindings are avaliable by importing the library as a Python module containing a Python class ```Pattern```, which can be initialised with an RLE/apgcode and a rule (default ```b3s23```). 
...
- `hashsoup(rule, instring, sym)`: Produces a Pattern instance in `rule` based on the SHA-256 hash of `instring` and the specified symmetry. Also available from C++.
