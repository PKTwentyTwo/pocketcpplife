...
## Introduction
This project was mainly written as a way to develop my proficiency in C++ by writing a header-only library. I may have a practical use for it in the future, but only on non-x86 systems, as the much faster [lifelib](https://gitlab.com/apgoucher/lifelib) can be used instead.
...
You can change certain parameters in the file [userparams.hpp](/userparams.hpp), which defines values such as the unordered map to use (default std::unordered_map) and maximum period for periodicity detection.
